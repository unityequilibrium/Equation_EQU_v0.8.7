...
def test_neutron_lifetime():
    ...
    print(f"    τ = {beam_2024['value_s']:.1f} ± {beam_2024['stat_uncertainty_s']:.1f} s")
...
def test_ckm_vud():
    ...
    print(f"  Deficit: {CKM_UNITARITY['deficit_from_1']:.4f} ({CKM_UNITARITY['status']})")
...
def run_all_tests():
    ...
    print("-" * 75)
    print(f"{'Neutron Lifetime Puzzle':<35} {'DOCUMENTED':<15} {'Δτ = ' + str(metric1) + ' s':<25}")
    print(f"{'CKM |V_ud|':<35} {'PASS' if pass2 else 'CLOSE':<15} {f'{metric2:.2f}% error':<25}")
    print(
