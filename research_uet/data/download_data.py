...
def download_sparc(output_dir: Path = None):
    ...
    try:
        response = requests.get(SPARC_URL, timeout=60)
        if response.status_code == 200:
...
def main():
    ...
    print("=" * 60)
    print("UET DATA DOWNLOAD SCRIPT")
    ...
    if not any([args.all, args.pdg, args.nndc, args.sparc, args.bibtex]):
        print("\nUsage: python download_data.py --all")
        print("       python download_data.py --pdg")
        print("       python download_data.py --sparc")
        print("       python download_data.py --bibtex")
